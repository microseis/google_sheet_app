...
def home(request):
    ...
    currency_xml = urllib.request.urlopen('http://www.cbr.ru/scripts/XML_daily.asp?date_req=' + str(cur_date))
    ...
    #df = pd.DataFrame(data=zip(ord_costs, ord_dates))
    #df.rename(columns={0: 'Стоимость, $', 1: 'Дата поставки'}, inplace=True)
    sum = ord_costs.aggregate(sum = Sum('ord_cost'))['sum']

    #построение графика
    line_chart = pygal.Line(y_title='Стоимость, $', x_title='Дата поставки', x_label_rotation=-90, height=400)
...
# функция для чтения google spreadsheet таблицы
def sheet_read():
...
def cur_rate_identify(currency_xml):
...
def update_db(item, rate):
    p, _ = Order.objects.update_or_create(ord_num=item[1],
                                          defaults={'ord_id': item[0],
                                                    'ord_cost': item[2],
                                                    'ord_date': datetime.strptime(item[3], '%d.%m.%Y'),
                                                    'ord_cost_rub': round(float(item[2]) *
                                                                          float(rate.replace(',', '.')), 2)}
